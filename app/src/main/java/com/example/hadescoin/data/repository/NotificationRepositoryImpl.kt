...
class NotificationRepositoryImpl(
    ...
    override fun observeNotifications(
        ...
        return dataSource.observeNotifications(phoneNumber) { snapshots ->
            val list = snapshots.map { snapshot ->
                AppNotification(
                    id = snapshot.key ?: "",
                    phoneNumber = phoneNumber,
                    title = snapshot.child("title").getValue(String::class.java) ?: "",
    ...
    override fun stopObserving(phoneNumber: String, subscription: Any) {
        if (subscription is ValueEventListener) {
            dataSource.removeNotificationsListener(phoneNumber, subscription)
        }
    }
}
